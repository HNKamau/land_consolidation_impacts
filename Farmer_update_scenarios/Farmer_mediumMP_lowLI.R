...
# make_variables(estimate_read_csv("farmerLC_size1.csv",
#                                  sep = ";"))
# 
...
crop_benefit1 <- 
  ...
  vv(med_maize_price_kes_kg, gen_CV, n_years) 

# natural capital  
adjusted_value_of_assets1 <-vv(value_of_farm_assets, gen_CV, n_years,
                              relative_trend = inflation_rate)

# Food cost saved from food accessible from the farm 
annual_saved_food_cost1 <- vv(saved_food_cost_pm, gen_CV, n_years, 
                             relative_trend = inflation_rate) * 12 

farmer_pre_interv_benefit1 <- crop_benefit1 + adjusted_value_of_assets1 +
                          annual_saved_food_cost1

# B) Cost of the farmer before the intervention ####
# medical Bills from farm related stresses
 medical_bills1 <- prop_hhincome_spent_on_hospital/100 * 
                      vv(hh_income_pa, gen_CV,n_years)
 
# production costs for the farm
 farming_costs1 <- vv(production_costs_saved_acre, gen_CV, n_years, 
                     relative_trend = inflation_rate)*
                   ha_per_hh * ha_acre_conversion * no_of_seasons
 
 # planning cost involves legal fee and the cost of knowledge acquisition
 farmer_plan_cost1  <- planning_cost
 
 farmer_pre_interv_cost1 <- medical_bills1 + farming_costs1 + farmer_plan_cost1
                                                           
# C) Farmer  costs  with Intervention ####
 farmer_one_time_cost1 <- cost_of_disruption_kes + # Damages on physical assets
...
farmer_recurring_cost1 <- adjusted_value_of_assets1 + # loss
                    annual_saved_food_cost1 # Food unavailable from farm directly
  
farmer_cost_interv1 <-  farmer_one_time_cost1 +  farmer_plan_cost1 +
                  farmer_recurring_cost1

# D) Farmer Benefits with Intervention ####
# farmer's annual compensation aka lessor fee
lessor_fee10 <-vv(compensation_income_pm_acre, gen_CV, n_years)* 2 * 
             ha_per_hh * ha_acre_conversion

#off farm employments and wages
off_farm_income1 <-vv(off_farm_income_kes_pm, gen_CV, n_years) *12

#income saved: production costs for farming
production_costs_saved1 <- farming_costs1
# hospital bills saved (proxy for long term health) from farm related stresses
medical_bills_saved1 <- medical_bills1  
...
adjusted_social_time <- vv(social_time_hr_day, gen_CV, n_years) *
                      (1-social_time_risk)
social_cohesion1 <- adjusted_social_time * vv(labour_cost_kes_hr, gen_CV, 
...
better_childhood1 <- child_farm_time_hr_week * children_per_hh * 
                        n_weeks_child_farm_time *
                        vv(labour_cost_kes_hr, gen_CV, n_years, 
                           relative_trend = inflation_rate ) 

farmer_benefit_interv10 <-
  lessor_fee10 + off_farm_income1 + production_costs_saved1 + 
  sale_of_hh_items_not_needed +  medical_bills_saved1 + social_cohesion1 + 
  better_childhood1

# E) Calculate net benefit ####
farmer_pre_interv_netbenefit_kes10 <- farmer_pre_interv_benefit1 -
                                            farmer_pre_interv_cost1

result_pre_intervention10 <- farmer_pre_interv_netbenefit_kes10/currency_change
    
farmer_interv_netbenefit_kes10 <- farmer_benefit_interv10 - farmer_cost_interv1

result_intervention10 <- farmer_interv_netbenefit_kes10/currency_change

# Calculate categorized costs and benefits ####
# Categories include 1) Food costs not saved 
food_cost_saved_npv1 <- discount(annual_saved_food_cost1, discount_rate,
                                calculate_NPV = TRUE)
# 2) Natural assets lost 
natural_assets_npv1 <- discount(adjusted_value_of_assets1, discount_rate,
                               calculate_NPV = T)
# 3) Planning cost 
...
lease_npv10 <- discount(lessor_fee10, discount_rate, calculate_NPV = T)
# 7) Production costs saved
prdn_costs_npv1 <- discount(farming_costs1, discount_rate, calculate_NPV = T)
# 8) Alternative employment
alt_income_npv1 <- discount(off_farm_income1, discount_rate, calculate_NPV = T)
# 9) Better childhood
better_childhood_npv1 <- discount(better_childhood1, discount_rate,
                                 calculate_NPV = T)
# 10) Social cohesion
social_npv1 <- discount(social_cohesion1, discount_rate, calculate_NPV = T)
# 11) Medical bills saved
medical_npv1 <- discount(medical_bills_saved1, discount_rate, calculate_NPV = T)
# 12) Annual crop yield
crop_npv1 <- discount(crop_benefit1, discount_rate, calculate_NPV = T)


# NPV ####
NPV_interv_kes10 <- discount(farmer_interv_netbenefit_kes10, discount_rate,
                           calculate_NPV = TRUE)
NPV_intervention10 <- 
  discount(result_intervention10, discount_rate, calculate_NPV = T)

NPV_n_interv_kes10 <- discount(farmer_pre_interv_netbenefit_kes10, discount_rate,
                             calculate_NPV = TRUE)

NPV_n_intervention10 <- 
    discount(result_pre_intervention10, discount_rate, calculate_NPV = T)

# Benefit cost ratio ####
# without intervention
npv_farmer_pre_interv_benefit10 <- discount(farmer_pre_interv_benefit1, discount_rate,
                                          calculate_NPV = T)

npv_farmer_pre_interv_cost10 <- discount(farmer_pre_interv_cost1, discount_rate,
                                       calculate_NPV = T)
bcr_n_interv1 <-npv_farmer_pre_interv_benefit10/npv_farmer_pre_interv_cost10
# with intervention
npv_farmer_benefit_interv1 <- discount(farmer_benefit_interv10, discount_rate, 
                                      calculate_NPV = T)
npv_farmer_cost_interv1 <- discount(farmer_cost_interv1, discount_rate, 
                                   calculate_NPV = T)
bcr_interv1 <- npv_farmer_benefit_interv1/npv_farmer_cost_interv1

#    return(list(Interv_NPV_kes10 = NPV_interv_kes10,
...
#                                     vars = c("Interv_NPV_kes10", "No_Interv_NPV_kes10"),
#                                     method = 'smooth_simple_overlay',
#                                     base_size = 8)
# 
# decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_results,
#                                     vars = "NPV_decision_kes10",
#                                     method = 'smooth_simple_overlay')
# 
...
#                 input_table = read.csv("Input_tables/farmer_only_input_table.csv", 
#                                        sep =";"),
...
# 
# outcome %>% 
#   select(Cashflow_decision_s1l0:Cashflow_decision_do25) %>% 
