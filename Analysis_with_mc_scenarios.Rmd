...
<!-- Data might be 'expert estimates' or something similar -->
...
<!-- not clear what NB is here  -->
<!-- text without spaces becomes part of the previous argument (indent is not enough) -->

Below is a series of equations that define the above costs, benefits, and risks for both options  in the function `farmer`. The calculations help us find out land consolidation effects by calculating the difference between the options. This difference is the **decision** that can influence viability of land consolidation for the farmers. To calculate the decision, the below equation takes a wide range of inputs, which means, uncertainty is unavoidable.  

<!-- I will be surprised if <br> works, generaly we need latex arguments -->
<br>
...
# I had to do some work to get the separators to work here

make_variables(decisionSupport::estimate_read_csv(paste("Input_tables/land_consolidation_input.csv", sep=",")))

```

<!-- I find it is usually better to move large r scripts to a .R file and use them in the RMD with 'source'  -->
...
mcSimulation_results <-  decisionSupport::mcSimulation(
  ...
  # CW NOTE updated the file here (replace ; with ')
...
fig2a <- decisionSupport::plot_pls(pls_result,
    ...
    # CW Note I changed this to work on my machone
                          threshold = 1,
...
fig2b<- plot_evpi(evpi, decision_vars = "net_benefit_lc_kes", 
                  input_table = read.csv("Input_tables/land_consolidation_input.csv", sep = ","),
                  # CW Note changed this .csv too
                  bar_color = "#1a80bb",
                  x_axis_name = "EVPI in KES",
                  new_names = "") +
  scale_y_discrete(labels = function(y) str_wrap(y, width = 20)) +
...
```{r, warning=FALSE, message =FALSE, include = FALSE,  fig.cap="***Figure 0. *** *The decision outcome of land consolidation when the distribution of maize prices is changed from uniform to posnorm. Outcome based on 1000 model runs of land consolidation model function.*" }
...
netbenefit_lc[,grep("_change$", colnames(netbenefit_lc))] %>% 
  ...
  stat_summary(geom = "col", fun = median) +
  xlab("Market price scenarios") + ylab("NPV per hectare in million KES")+
  coord_flip()+ theme_bw()+
