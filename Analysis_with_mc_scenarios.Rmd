...
ggsave("figures/overall_distribution.png", dpi = 300)
...
ggsave("figures/decision_outcome.png", dpi = 300)   
...
ggsave("figures/VIP.png",dpi = 300)
...
ggsave("figures/EVPI.png",dpi = 300)
...
list_of_dfs <- lapply(seq_along(list_of_dfs), function(i) {
  df <- list_of_dfs[[i]]
  df <- df %>%
    mutate(Supply.Volume = as.numeric(Supply.Volume))  # Convert to numeric
  df$source_file <- basename(csv_files[i])  # Adding file name as a new column to track the source
  return(df) })
...
combined_df <- combined_df %>%
  mutate(
    ...
    Wholesale = gsub(",", "", Wholesale),             # Remove commas
...
netbenefit_lc_all %>% 
 ...
 ggsave("figures/net_benefit_scenarios.png", dpi = 300) 
...
netbenefit_lc[,grep("_change$", colnames(netbenefit_lc))] %>% 
 ...
 ggsave("figures/market_scenario_change.png", dpi = 300)
