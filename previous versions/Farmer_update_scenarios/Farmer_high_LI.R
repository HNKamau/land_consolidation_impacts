# # LEASE INCOME CLASS 3 WITH ORIGINAL LAND SIZE RANGE
...
# {
  ...
  lessor_fee <-vv(compensation_income2_pm_acre, gen_CV, n_years)* 2 * 
  ...
  farmer_pre_interv_netbenefit_kes3 <- farmer_pre_interv_benefit -
    farmer_pre_interv_cost
  
  result_pre_intervention3 <- farmer_pre_interv_netbenefit_kes3/currency_change
  
  farmer_interv_netbenefit_kes3 <- farmer_benefit_interv3 - farmer_cost_interv
  
  result_intervention3 <- farmer_interv_netbenefit_kes3/currency_change
  ...
  # NPV ####
  NPV_interv_kes3 <- discount(farmer_interv_netbenefit_kes3, discount_rate,
                              calculate_NPV = TRUE)
  NPV_intervention3 <- 
    discount(result_intervention3, discount_rate, calculate_NPV = T)
  
  NPV_n_interv_kes3 <- discount(farmer_pre_interv_netbenefit_kes3, discount_rate,
                                calculate_NPV = TRUE)
  
  NPV_n_intervention3 <- 
    discount(result_pre_intervention3, discount_rate, calculate_NPV = T)
  ...
  # with intervention
  npv_farmer_benefit_interv <- discount(farmer_benefit_interv3, discount_rate, 
                                        calculate_NPV = T)
...
#   return(list(Interv_NPV_kes3 = NPV_interv_kes3,
#               No_Interv_NPV_kes3 = NPV_n_interv_kes3,
#               NPV_decision_kes3 = NPV_interv_kes3 - NPV_n_interv_kes3,
#               NPV_decision_do = NPV_intervention3 - NPV_n_intervention3,
...
#                            decision_vars = "NPV_decision_kes3",
#                            input_table =read.csv("farmerLC_size0.csv", sep=";"),
#                            bar_color = "#1a80bb") 
#   
# 
...
#                 EVPIresults = evpi,
#                 decision_var_name = "NPV_decision_kes3",
#                 cashflow_var_name = "Cashflow_decision_L3",
#                 base_size = 7)
# 
# 
...
# str(outcome)
# 
# outcome <- outcome %>% mutate(NPV_per_ha = Interv_NPV_kes3/ha_per_hh, 
#                               No_NPV_per_ha = No_Interv_NPV_kes3/ha_per_hh,
#                               NPV_per_ha_decision = NPV_decision_kes3/ha_per_hh )
...
# outcome %>% 
#   select(NPV_decision_kes3) %>% 
#   pivot_longer(., cols ="NPV_decision_kes3",
#                names_to = "variables", values_to = "project_outcome") %>% 
...
#          starts_with('Cashflow_decision_L3')) %>% 
#   mutate(Year = as.numeric(gsub('Cashflow_decision_L3', '', Year))) %>% 
#   group_by(Year) %>% 
#   summarise(
...
# 
# evpiplot <- evpidata %>%
#   select(NPV_decision_kes3.EVPI_do, NPV_decision_kes3.variable)  %>% 
#   filter(NPV_decision_kes3.EVPI_do > 0)  %>%  
#   ggplot(aes( y = NPV_decision_kes3.variable, x = NPV_decision_kes3.EVPI_do))+
...
#                "compensation_income2_pm_acre" = str_wrap("Lease income (KES/acre)",
#                width =10))) +
#   theme_bw()+
