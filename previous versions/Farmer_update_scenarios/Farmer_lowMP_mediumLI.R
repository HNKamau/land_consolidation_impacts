...
lessor_fee01 <-vv(compensation_income1_pm_acre, gen_CV, n_years)* 2 * 
...
farmer_benefit_interv01 <-
  lessor_fee01 + off_farm_income + production_costs_saved + 
...
farmer_interv_netbenefit_kes01 <- farmer_benefit_interv01 - farmer_cost_interv

result_intervention <- farmer_interv_netbenefit_kes01/currency_change
...
lease_npv <- discount(lessor_fee01, discount_rate, calculate_NPV = T)
...
# NPV ####
NPV_interv_kes01 <- discount(farmer_interv_netbenefit_kes01, discount_rate,
                           calculate_NPV = TRUE)
NPV_intervention <- 
...
# with intervention
npv_farmer_benefit_interv <- discount(farmer_benefit_interv01, discount_rate, 
                                      calculate_NPV = T)
...
#                Interv_NPV_kes = NPV_interv_kes01,
#                No_Interv_NPV = NPV_n_intervention01,
#                No_Interv_NPV_kes = NPV_n_interv_kes,
#                NPV_decision_kes = NPV_interv_kes01 - NPV_n_interv_kes,
#                NPV_decision_do = NPV_intervention - NPV_n_intervention01,
...
#          starts_with('Cashflow_decision_s0l1')) %>% 
#   mutate(Year = as.numeric(gsub('Cashflow_decision_s0l1', '', Year))) %>% 
#   group_by(Year) %>% 
#   summarise(
