...
# #LEASE INCOME CLASS 1 WITH ORIGINAL LAND SIZE RANGE
...
farmer_pre_interv_netbenefit_kes1 <- farmer_pre_interv_benefit -
                                            farmer_pre_interv_cost

result_pre_intervention1 <- farmer_pre_interv_netbenefit_kes1/currency_change
    
farmer_interv_netbenefit_kes1 <- farmer_benefit_interv1 - farmer_cost_interv
...
# with intervention
npv_farmer_benefit_interv <- discount(farmer_benefit_interv1, discount_rate, 
                                      calculate_NPV = T)
...
#                NPV_decision_do = NPV_intervention1 - NPV_n_intervention1,
...
#                            decision_vars = "NPV_decision_kes1",
#                            input_table =read.csv("farmerLC_size0.csv", sep=";"),
#                            bar_color = "#1a80bb") 
#   
# 
...
#                 EVPIresults = evpi,
#                 decision_var_name = "NPV_decision_kes1",
#                 cashflow_var_name = "Cashflow_decision_L1",
#                 base_size = 7)
# 
# 
...
# str(outcome)
# 
# outcome <- outcome %>% mutate(NPV_per_ha = Interv_NPV_kes1/ha_per_hh, 
#                               No_NPV_per_ha = No_Interv_NPV_kes1/ha_per_hh,
#                               NPV_per_ha_decision = NPV_decision_kes1/ha_per_hh )
...
# outcome %>% 
#   select(NPV_decision_kes1) %>% 
#   pivot_longer(., cols ="NPV_decision_kes1",
#                names_to = "variables", values_to = "project_outcome") %>% 
...
#          starts_with('Cashflow_decision_L1')) %>% 
#   mutate(Year = as.numeric(gsub('Cashflow_decision_L1', '', Year))) %>% 
#   group_by(Year) %>% 
#   summarise(
...
# 
# evpiplot <- evpidata %>%
#   select(NPV_decision_kes1.EVPI_do, NPV_decision_kes1.variable)  %>% 
#   filter(NPV_decision_kes1.EVPI_do > 0)  %>%  
#   ggplot(aes( y = NPV_decision_kes1.variable, x = NPV_decision_kes1.EVPI_do))+
