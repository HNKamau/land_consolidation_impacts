...
# make_variables(estimate_read_csv("Input_tables/farmer_only_input_table.csv",
#                                  sep = ";"))
# 
...
#   
  ...
  crop_benefit2 <- 
    ...
    vv(high_maize_price_kes_kg, gen_CV, n_years) 
  
  # natural capital  
  adjusted_value_of_assets2 <-vv(value_of_farm_assets, gen_CV, n_years,
                                 relative_trend = inflation_rate)
  
  # Food cost saved from food accessible from the farm 
  annual_saved_food_cost2 <- vv(saved_food_cost_pm, gen_CV, n_years, 
                                relative_trend = inflation_rate) * 12 
  
  farmer_pre_interv_benefit2 <- crop_benefit2 + adjusted_value_of_assets2 +
    annual_saved_food_cost2
  
  # B) Cost of the farmer before the intervention ####
  # medical Bills from farm related stresses
  medical_bills2 <- prop_hhincome_spent_on_hospital/100 * 
    vv(hh_income_pa, gen_CV,n_years)
  
  # production costs for the farm
  farming_costs2 <- vv(production_costs_saved_acre, gen_CV, n_years, 
                       relative_trend = inflation_rate)*
    ha_per_hh * ha_acre_conversion * no_of_seasons
  
  # planning cost involves legal fee and the cost of knowledge acquisition
  farmer_plan_cost2  <- planning_cost
  
  farmer_pre_interv_cost2 <- medical_bills2 + farming_costs2 + farmer_plan_cost2
  
  # C) Farmer  costs  with Intervention ####
  farmer_one_time_cost2 <- cost_of_disruption_kes + # Damages on physical assets
  ...
  farmer_recurring_cost2 <- adjusted_value_of_assets2 + # loss
    annual_saved_food_cost2 # Food unavailable from farm directly
  
  farmer_cost_interv2 <-  farmer_one_time_cost2 +  farmer_plan_cost2 +
    farmer_recurring_cost2
  
  # D) Farmer Benefits with Intervention ####
  # farmer's annual compensation aka lessor fee
  lessor_fee22 <-vv(compensation_income2_pm_acre, gen_CV, n_years)* 2 * 
    ha_per_hh * ha_acre_conversion
  
  #off farm employments and wages
  off_farm_income2 <-vv(off_farm_income_kes_pm, gen_CV, n_years) *12
  
  #income saved: production costs for farming
  production_costs_saved2 <- farming_costs2
  # hospital bills saved (proxy for long term health) from farm related stresses
  medical_bills_saved2 <- medical_bills2  
  ...
  adjusted_social_time <- vv(social_time_hr_day, gen_CV, n_years) *
    (1-social_time_risk)
  social_cohesion2 <- adjusted_social_time * vv(labour_cost_kes_hr, gen_CV, 
  ...
  better_childhood2 <- child_farm_time_hr_week * children_per_hh * 
    n_weeks_child_farm_time *
    vv(labour_cost_kes_hr, gen_CV, n_years, 
       relative_trend = inflation_rate ) 
  
  farmer_benefit_interv22 <-
    lessor_fee22 + off_farm_income2 + production_costs_saved2 + 
    sale_of_hh_items_not_needed +  medical_bills_saved2 + social_cohesion2 + 
    better_childhood2
  
  # E) Calculate net benefit ####
  farmer_pre_interv_netbenefit_kes22 <- farmer_pre_interv_benefit2 -
    farmer_pre_interv_cost2
  
  result_pre_intervention22 <- farmer_pre_interv_netbenefit_kes22/currency_change
  
  farmer_interv_netbenefit_kes22 <- farmer_benefit_interv22 - farmer_cost_interv2
  
  result_intervention22 <- farmer_interv_netbenefit_kes22/currency_change
  
  # Calculate categorized costs and benefits ####
  # Categories include 1) Food costs not saved 
  food_cost_saved_npv2 <- discount(annual_saved_food_cost2, discount_rate,
                                   calculate_NPV = TRUE)
  # 2) Natural assets lost 
  natural_assets_npv2 <- discount(adjusted_value_of_assets2, discount_rate,
                                  calculate_NPV = T)
  # 3) Planning cost 
  ...
  lease_npv22 <- discount(lessor_fee22, discount_rate, calculate_NPV = T)
  # 7) Production costs saved
  prdn_costs_npv2 <- discount(farming_costs2, discount_rate, calculate_NPV = T)
  # 8) Alternative employment
  alt_income_npv2 <- discount(off_farm_income2, discount_rate, calculate_NPV = T)
  # 9) Better childhood
  better_childhood_npv2 <- discount(better_childhood2, discount_rate,
                                    calculate_NPV = T)
  # 10) Social cohesion
  social_npv2 <- discount(social_cohesion2, discount_rate, calculate_NPV = T)
  # 11) Medical bills saved
  medical_npv2 <- discount(medical_bills_saved2, discount_rate, calculate_NPV = T)
  # 12) Annual crop yield
  crop_npv2 <- discount(crop_benefit2, discount_rate, calculate_NPV = T)
  
  
  # NPV ####
  NPV_interv_kes22 <- discount(farmer_interv_netbenefit_kes22, discount_rate,
                              calculate_NPV = TRUE)
  NPV_intervention22 <- 
    discount(result_intervention22, discount_rate, calculate_NPV = T)
  
  NPV_n_interv_kes22 <- discount(farmer_pre_interv_netbenefit_kes22, discount_rate,
                                calculate_NPV = TRUE)
  
  NPV_n_intervention22 <- 
    discount(result_pre_intervention22, discount_rate, calculate_NPV = T)
  
  # Benefit cost ratio ####
  # without intervention
  npv_farmer_pre_interv_benefit2 <- discount(farmer_pre_interv_benefit2, discount_rate,
                                             calculate_NPV = T)
  
  npv_farmer_pre_interv_cost2 <- discount(farmer_pre_interv_cost2, discount_rate,
                                          calculate_NPV = T)
  bcr_n_interv2 <-npv_farmer_pre_interv_benefit2/npv_farmer_pre_interv_cost2
  # with intervention
  npv_farmer_benefit_interv2 <- discount(farmer_benefit_interv22, discount_rate, 
                                         calculate_NPV = T)
  npv_farmer_cost_interv2 <- discount(farmer_cost_interv2, discount_rate, 
                                      calculate_NPV = T)
  bcr_interv2 <- npv_farmer_benefit_interv2/npv_farmer_cost_interv2
  
  #    return(list(Interv_NPV_kes22 = NPV_interv_kes22,
