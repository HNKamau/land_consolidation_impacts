# 
# 
# # LEASE INCOME PER ACRE CLASS 2 AND THE ORIGINAL LAND SIZE RANGE 
# 
# 
...
lessor_fee <-vv(compensation_income1_pm_acre, gen_CV, n_years)* 2 * 
...
farmer_pre_interv_netbenefit_kes2 <- farmer_pre_interv_benefit -
  farmer_pre_interv_cost

result_pre_intervention2 <- farmer_pre_interv_netbenefit_kes2/currency_change

farmer_interv_netbenefit_kes2 <- farmer_benefit_interv2 - farmer_cost_interv
...
# with intervention
npv_farmer_benefit_interv <- discount(farmer_benefit_interv2, discount_rate, 
                                      calculate_NPV = T)
...
#                NPV_decision_do = NPV_intervention2 - NPV_n_intervention2,
...
#                            decision_vars = "NPV_decision_kes2",
#                            input_table =read.csv("farmerLC_size0.csv", sep=";"),
#                            bar_color = "#1a80bb") 
#   
# 
...
# str(outcome)
# 
# outcome <- outcome %>% mutate(NPV_per_ha = Interv_NPV_kes2/ha_per_hh, 
#                               No_NPV_per_ha = No_Interv_NPV_kes2/ha_per_hh,
#                               NPV_per_ha_decision = NPV_decision_kes2/ha_per_hh )
...
# outcome %>% 
#   select(NPV_decision_kes2) %>% 
#   pivot_longer(., cols ="NPV_decision_kes2",
#                names_to = "variables", values_to = "project_outcome") %>% 
...
#          starts_with('Cashflow_decision_L2')) %>% 
#   mutate(Year = as.numeric(gsub('Cashflow_decision_L2', '', Year))) %>% 
#   group_by(Year) %>% 
#   summarise(
...
# 
# evpiplot <- evpidata %>%
#   select(NPV_decision_kes2.EVPI_do, NPV_decision_kes2.variable)  %>% 
#   filter(NPV_decision_kes2.EVPI_do > 0)  %>%  
#   ggplot(aes( y = NPV_decision_kes2.variable, x = NPV_decision_kes2.EVPI_do))+
...
#                "compensation_income1_pm_acre" = str_wrap("Lease income (KES/acre)",
#                width =10))) +
#   theme_bw()+
