...
lessor_fee11 <-vv(compensation_income1_pm_acre, gen_CV, n_years)* 2 * 
...
farmer_benefit_interv11 <-
  lessor_fee11 + off_farm_income1 + production_costs_saved1 + 
  sale_of_hh_items_not_needed +  medical_bills_saved1 + social_cohesion1 + 
  better_childhood1

# E) Calculate net benefit ####
farmer_pre_interv_netbenefit_kes11 <- farmer_pre_interv_benefit1 -
  farmer_pre_interv_cost1

result_pre_intervention11 <- farmer_pre_interv_netbenefit_kes11/currency_change

farmer_interv_netbenefit_kes11 <- farmer_benefit_interv11 - farmer_cost_interv1

result_intervention11 <- farmer_interv_netbenefit_kes11/currency_change
...
lease_npv11 <- discount(lessor_fee11, discount_rate, calculate_NPV = T)
...
# NPV ####
NPV_interv_kes11 <- discount(farmer_interv_netbenefit_kes11, discount_rate,
                            calculate_NPV = TRUE)
NPV_intervention11 <- 
  discount(result_intervention11, discount_rate, calculate_NPV = T)

NPV_n_interv_kes11 <- discount(farmer_pre_interv_netbenefit_kes11, discount_rate,
                              calculate_NPV = TRUE)

NPV_n_intervention11 <- 
  discount(result_pre_intervention11, discount_rate, calculate_NPV = T)

# Benefit cost ratio ####
# without intervention
npv_farmer_pre_interv_benefit11 <- discount(farmer_pre_interv_benefit1, discount_rate,
                                           calculate_NPV = T)

npv_farmer_pre_interv_cost11 <- discount(farmer_pre_interv_cost1, discount_rate,
                                        calculate_NPV = T)
bcr_n_interv1 <-npv_farmer_pre_interv_benefit11/npv_farmer_pre_interv_cost11
# with intervention
npv_farmer_benefit_interv1 <- discount(farmer_benefit_interv11, discount_rate, 
                                       calculate_NPV = T)
...
#    return(list(Interv_NPV_kes11 = NPV_interv_kes11,
...
# 
# outcome %>% 
#   select(Cashflow_decision_s1l1:Cashflow_decision_do25) %>% 
