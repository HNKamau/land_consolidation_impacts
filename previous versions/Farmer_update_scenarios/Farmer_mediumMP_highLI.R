...
lessor_fee12 <-vv(compensation_income2_pm_acre, gen_CV, n_years)* 2 * 
...
farmer_benefit_interv12 <-
  lessor_fee12 + off_farm_income1 + production_costs_saved1 + 
  sale_of_hh_items_not_needed +  medical_bills_saved1 + social_cohesion1 + 
  better_childhood1

# E) Calculate net benefit ####
farmer_pre_interv_netbenefit_kes12 <- farmer_pre_interv_benefit1 -
  farmer_pre_interv_cost1

result_pre_intervention12 <- farmer_pre_interv_netbenefit_kes12/currency_change

farmer_interv_netbenefit_kes12 <- farmer_benefit_interv12 - farmer_cost_interv1

result_intervention12 <- farmer_interv_netbenefit_kes12/currency_change
...
lease_npv12 <- discount(lessor_fee12, discount_rate, calculate_NPV = T)
...
# NPV ####
NPV_interv_kes12 <- discount(farmer_interv_netbenefit_kes12, discount_rate,
                             calculate_NPV = TRUE)
NPV_intervention12 <- 
  discount(result_intervention12, discount_rate, calculate_NPV = T)

NPV_n_interv_kes12 <- discount(farmer_pre_interv_netbenefit_kes12, discount_rate,
                               calculate_NPV = TRUE)

NPV_n_intervention12 <- 
  discount(result_pre_intervention12, discount_rate, calculate_NPV = T)

# Benefit cost ratio ####
# without intervention
npv_farmer_pre_interv_benefit1 <- discount(farmer_pre_interv_benefit1, discount_rate,
                                           calculate_NPV = T)

npv_farmer_pre_interv_cost1 <- discount(farmer_pre_interv_cost1, discount_rate,
                                        calculate_NPV = T)
bcr_n_interv1 <-npv_farmer_pre_interv_benefit1/npv_farmer_pre_interv_cost1
# with intervention
npv_farmer_benefit_interv1 <- discount(farmer_benefit_interv12, discount_rate, 
                                       calculate_NPV = T)
...
#   return(list(Interv_NPV_kes12 = NPV_interv_kes12,
...
#                                     vars = c("Interv_NPV_kes12", "No_Interv_NPV_kes12"),
#                                     method = 'smooth_simple_overlay',
#                                     base_size = 8)
# 
# decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_results,
#                                     vars = "NPV_decision_kes12",
...
# 
# outcome %>% 
#   select(Cashflow_decision_s1l2:Cashflow_decision_do25) %>% 
